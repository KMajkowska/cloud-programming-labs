...
resource "aws_security_group" "tic-tac-toe-security-group" {
    name        = "tic-tac-toe-security-group"
    vpc_id      = aws_vpc.tic-tac-toe-vpc.id
...
resource "aws_internet_gateway" "tic-tac-toe-internet-gateway" {
    vpc_id = aws_vpc.tic-tac-toe-vpc.id
    tags = {
        Name = "tic-tac-toe-internet-gateway"
    }
}

# Tworzy tabelę routingu dla VPC, dodając trasę domyślną przez bramę 
resource "aws_route_table" "tic-tac-toe-routing" { 
    vpc_id = aws_vpc.tic-tac-toe-vpc.id

    route {
        cidr_block = "0.0.0.0/0"
        gateway_id = aws_internet_gateway.tic-tac-toe-internet-gateway.id
    }

    tags = {
        Name = "tic-tac-toe-routing"
    }
}

# Wiąże tabelę routingu z podsiecią, umożliwiając jej dostęp do internetu.
resource "aws_route_table_association" "tic-tac-toe-routing-table" {
